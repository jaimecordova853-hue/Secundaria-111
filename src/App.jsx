...
import {
  ...
  Share2, X, Edit3, BarChart2, Trash2,
...
// Analytics solo en navegador y con blindaje
let analytics;
...
const Button = ({ children, onClick, variant = "primary", className = "", icon: Icon, disabled, title }) => {
  const baseStyle =
    "px-3 py-2 rounded-lg font-medium transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed text-sm";
  const variants = {
...
const StudentReportModal = ({
  ...
  const metaColor =
    student.metas === "Alcanzada"
      ? "text-green-700 bg-green-50"
      : student.metas === "No Logrado"
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="bg-blue-50 p-3 rounded-lg border border-blue-100">
            <p className="text-xs font-bold text-blue-600 uppercase mb-1">
              Actividad: {activityInfo.title || "Sin título"}
            </p>
            <p className="text-sm text-gray-600 italic">
              {activityInfo.description || "Sin descripción"}
            </p>
          </div>

          <div className="grid grid-cols-2 gap-4 text-sm">
            <div className="p-2 bg-gray-50 rounded">
              <span className="block text-gray-500 text-xs">Asistencia</span>
              <span className={`font-bold ${student.asistencia === "Falta" ? "text-red-600" : "text-green-600"}`}>
                {student.asistencia}
              </span>
            </div>
            <div className="p-2 bg-gray-50 rounded">
              <span className="block text-gray-500 text-xs">Calificación</span>
              <span className="font-bold text-blue-600">{student.calificacion}/10</span>
            </div>
            <div className={`p-2 rounded ${metaColor} border border-opacity-20`}>
              <span className="block opacity-70 text-xs">Meta de Aprendizaje</span>
              <span className="font-bold">{student.metas}</span>
            </div>
            <div className="p-2 bg-gray-50 rounded">
              <span className="block text-gray-500 text-xs">Conducta</span>
              <span className="font-bold text-gray-800">{student.conducta}</span>
            </div>
            <div className="col-span-2 p-2 bg-gray-50 rounded flex justify-between">
              <span className="text-gray-500 text-xs">Actividad Entregada</span>
              <span className="font-bold text-gray-800">{student.entrega}%</span>
            </div>
          </div>

          {student.observaciones && (
            <div className="p-3 bg-yellow-50 border border-yellow-100 rounded text-sm text-yellow-800">
              <span className="font-bold">Observaciones:</span> {student.observaciones}
            </div>
          )}

          <div className="flex gap-2 pt-2">
            <Button className="flex-1" icon={Mail} onClick={() => onCopyEmail(student)}>
              Copiar Email
            </Button>
            <Button className="flex-1" variant="success" icon={Share2} onClick={() => onCopyShort(student)}>
              Resumen
            </Button>
          </div>
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      try {
        ...
        if (mounted) setUser({ uid: "anon-fallback" });
      }
    };
  ...
  const getDailyId = () =>
    `daily_${date}_${currentClass.turno}_${currentClass.grado}_${currentClass.grupo}_${currentClass.asignatura}`;
  ...
  const generateStudentReport = async () => {
    try {
      ...
      csv +=
        "Nombre del Alumno,Periodo,Calif. Promedio,Asistencias,Faltas,Retardos,Conducta Excelente (Días),Conducta Buena,Conducta Regular,Conducta Mala,Meta Alcanzada (Días),Meta En Proceso,Meta No Lograda,Entregas Promedio (%)\n";
  ...
  const generateGroupReport = async () => {
    try {
      ...
      const globalGrade = (
        studentsArray.reduce((acc, s) => acc + (s.days > 0 ? s.totalGrade / s.days : 0), 0) / totalStudents
      ).toFixed(2);

      const globalEntrega = (
        studentsArray.reduce((acc, s) => acc + (s.days > 0 ? s.totalEntrega / s.days : 0), 0) / totalStudents
      ).toFixed(2);
      ...
      const absencePercent =
        totalPossibleAttendances > 0
          ? ((totalFaltasGroup / totalPossibleAttendances) * 100).toFixed(2)
          : "0";
  ...
  const handleFileUpload = async (e) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        console.error(e);
        showNotification("Error subiendo lista", "danger");
      }
    };
...
${student.observaciones || ""}`;
        ...
        <Card className="w-full max-w-md p-6 space-y-6">
          ...
          <div className="space-y-4">
            <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-200">
              <label className="block text-sm font-bold text-yellow-800 mb-2 text-center">
                PERIODO
              </label>
              <div className="flex justify-center gap-4">
                {CONFIG.periodos.map((p) => (
                  <button
                    key={p}
                    onClick={() => setCurrentClass({ ...currentClass, periodo: p })}
                    className={`w-12 h-12 rounded-full font-bold text-lg border-2 ${
                      currentClass.periodo === p
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {CONFIG.turnos.map((t) => (
                  <button
                    key={t}
                    onClick={() => setCurrentClass({ ...currentClass, turno: t })}
                    className={`p-3 rounded-lg border text-sm ${
                      currentClass.turno === t ? "bg-blue-100 border-blue-500 font-bold" : ""
                    }`}
                  >
                    {t}
                  </button>
            ...
            <div className="flex gap-2">
              <div className="w-1/2">
                ...
                <select
                  ...
                  value={currentClass.grado}
                  onChange={(e) =>
                    setCurrentClass({ ...currentClass, grado: e.target.value, asignatura: "" })
                  }
                >
                  <option value="">...</option>
                  {CONFIG.grados.map((g) => (
              ...
              <div className="w-1/2">
                ...
                <select
                  className="block w-full p-2 border rounded-lg"
                  value={currentClass.grupo}
                  onChange={(e) =>
                    setCurrentClass({ ...currentClass, grupo: e.target.value })
                  }
                >
                  <option value="">...</option>
                  {CONFIG.grupos.map((g) => (
            ...
            {currentClass.grado && (
              <div>
                ...
                <select
                  className="block w-full p-2 border rounded-lg"
                  value={currentClass.asignatura}
                  onChange={(e) =>
                    setCurrentClass({ ...currentClass, asignatura: e.target.value })
                  }
                >
                  <option value="">Seleccione...</option>
                  {CONFIG.asignaturas[currentClass.grado].map((a) => (
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      <header className="bg-blue-800 text-white p-4 sticky top-0 z-30 shadow-lg">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <button onClick={() => setView("setup")} className="p-1 hover:bg-blue-700 rounded">
              <ChevronLeft />
            </button>
            <div>
          ...
          <div className="flex flex-col items-end">
            <div className="flex items-center bg-blue-900 rounded px-2 py-1 border border-blue-600">
              ...
              <input
                ...
                className="bg-transparent text-white font-mono text-sm outline-none cursor-pointer w-28"
              />
            </div>
            {isSyncing && (
              <span className="text-xs text-blue-200 mt-1 flex items-center">
                <RefreshCw size={10} className="animate-spin mr-1" /> Sincronizando...
              </span>
            )}
          </div>
        </div>
      ...
      {notification && (
        <div className="fixed top-24 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white px-4 py-2 rounded-full text-sm shadow-xl z-50 animate-bounce">
          {notification.msg}
        </div>
      )}
      ...
      {selectedStudent && (
        <StudentReportModal
          ...
          date={date}
          classNameInfo={getClassInfoString()}
          onClose={() => setSelectedStudent(null)}
          onCopyEmail={(s) => copyToClipboard(generateEmailText(s))}
          onCopyShort={(s) => copyToClipboard(`${s.name} - ${s.calificacion}`)}
        />
      )}
          ...
          <>
            <Card className="p-4 bg-blue-50 border-blue-200">
              <div className="flex items-start gap-3">
                <div className="mt-1 bg-blue-100 p-2 rounded-full text-blue-600 hidden sm:block">
                  <Edit3 size={20} />
                </div>
                <div className="flex-1 space-y-3">
                  ...
                  <input
                    ...
                    className="w-full p-2 border border-blue-200 rounded outline-none text-sm"
            ...
            <div className="flex flex-wrap gap-2 justify-between items-center bg-white p-2 rounded-lg border border-gray-200 shadow-sm sticky top-[4.5rem] z-20">
              <div className="flex gap-2 items-center flex-wrap">
                <label className="cursor-pointer bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-md text-sm font-medium flex items-center gap-1 border border-gray-300">
                  <Upload size={16} /> <span className="hidden sm:inline">CSV</span>
                  <input type="file" accept=".csv" className="hidden" onChange={handleFileUpload} />
                </label>

                {/* BOTÓN RECUPERACIÓN DE LISTA (arreglado) */}
                <button
                  onClick={forceLoadRoster}
                  className="bg-blue-50 hover:bg-blue-100 text-blue-700 px-3 py-2 rounded-md text-sm font-medium border border-blue-200"
                  title="Recuperar lista guardada"
                >
                  <Download size={16} />
                </button>
                ...
                <button
                  ...
                  className="bg-red-50 hover:bg-red-100 text-red-700 px-3 py-2 rounded-md text-sm font-medium border border-red-200"
                >
                  <Trash2 size={16} />
                </button>
                ...
                <Button onClick={exportDailyCSV} variant="success" icon={FileText} title="Reporte Diario en Excel">
                  <span className="hidden sm:inline">Diario</span>
                </Button>

                <Button onClick={generateStudentReport} variant="info" icon={UserCheck} title="Expediente Periodo Alumnos">
                  <span className="hidden sm:inline">Exp. Alumnos</span>
                </Button>

                <Button onClick={generateGroupReport} variant="warning" icon={PieChart} title="Expediente Periodo Grupo">
                  <span className="hidden sm:inline">Exp. Grupo</span>
                </Button>
              </div>
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  variant="secondary"
                >
                  {view === "dashboard" ? "Reportes" : "Lista"}
                </Button>
                <Button onClick={saveData} icon={Save} disabled={isSyncing}>
                  {isSyncing ? "..." : "Guardar"}
                </Button>
              </div>
            </div>
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {students.length === 0 && (
                  <div className="col-span-full text-center py-10 text-gray-400">
                    ...
                    <p>No hay alumnos cargados para este grupo.</p>
                    <p className="text-sm">
                      Sube un archivo CSV con los nombres o pulsa el botón azul de "Recuperar Lista".
                    </p>
                  </div>
                )}
                  ...
                  <Card key={student.id} className="p-4 relative">
                    <div className="flex justify-between items-start mb-3">
                      <h3
                        ...
                        className="font-bold text-gray-800 text-lg truncate cursor-pointer hover:text-blue-600"
                      >
                        {student.name}
                      </h3>
                    </div>
                    ...
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between bg-gray-50 p-1 rounded">
                        <button
                          onClick={() => updateStudent(student.id, "asistencia", "Asistencia")}
                        ...
                        <button
                          onClick={() => updateStudent(student.id, "asistencia", "Retardo")}
                          className={`flex-1 flex justify-center py-1 rounded ${
                            student.asistencia === "Retardo"
                        ...
                        <button
                          onClick={() => updateStudent(student.id, "asistencia", "Falta")}
                          className={`flex-1 flex justify-center py-1 rounded ${
                            student.asistencia === "Falta"
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          max="100"
                          step="10"
                          value={student.entrega}
                          onChange={(e) => updateStudent(student.id, "entrega", e.target.value)}
                          className="flex-1 h-2 bg-gray-200 rounded-lg cursor-pointer"
                        />
                        <span className="font-bold w-8 text-right text-blue-600">
                          {student.entrega}%
                        </span>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <select
                          ...
                          onChange={(e) => updateStudent(student.id, "conducta", e.target.value)}
                          className="flex-1 bg-white border rounded px-1 text-xs py-1"
                        >
                        ...
                        <div className="flex items-center gap-1 bg-blue-50 px-2 rounded border border-blue-100">
                          <span className="text-xs text-blue-800 font-bold">Calif:</span>
                          <input
                            type="number"
                            max="10"
                            min="0"
                            ...
                            onChange={(e) =>
                              updateStudent(student.id, "calificacion", e.target.value)
                            }
                            className="w-10 bg-transparent text-center font-bold text-blue-800 outline-none"
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  <Card key={student.id} className="p-4 flex flex-col justify-between bg-gray-50">
                    ...
                    <div className="mt-4 flex gap-2">
                      <Button
                        ...
                        icon={Mail}
                        onClick={() => copyToClipboard(generateEmailText(student))}
                      >
                        Correo
                      </Button>
                    </div>
